...
def get_args():
    ...
    parser.add_argument("--img_dir", type=str, required=True,
                        help="Input directory where images are placed")
...
def main():
    '''
    This script reads an image, predicts age from a model, draws a boundary with age and saves an image in args.output_dir
    '''
    args = get_args()
    ...
    with torch.no_grad():
        for img, name in image_generator:
            ...
            if len(detected) > 0:
                ...
                print("{} : {}".format(name, int(predicted_ages[0])))
